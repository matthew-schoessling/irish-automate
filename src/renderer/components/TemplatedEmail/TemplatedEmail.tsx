...
function TemplatedEmail({
    ...
    const createIntro = () => (
        <>
            ...
            <p>IrishAngels has recently come across an opportunity I thought you may be interested in. Please let me know if you'd like to discuss further!</p>
        </>
    );
    ...
    const createOppName = () => (
        <>
            {
                selectedOpportunity?.opportunity_website 
                ? <a href={`${selectedOpportunity?.opportunity_website}`} target="_blank">{selectedOpportunity?.name}</a> 
    ...
    const createOppMiniDetails = () => {
        var listOfMiniDetails = [];

        // Get round size and funding round details in coherent string together
        var oppRound = firstStageOfInvestmentDisplay(selectedOpportunity?.custom_fields ?? [], stageOfInvestmentOptions);
        ...
        // City of opportunity
        var cityString = selectedOpportunity?.custom_fields.find(cf => cf.custom_field_definition_id === 327449)?.value;
        if (cityString)
            listOfMiniDetails.push(cityString);

        // Industries
        var industriesString = getIndustries(selectedOpportunity?.custom_fields ?? [], industryOptions, ' and ');
        if (industriesString)
    ...
    const createDetails = () => {
        ...
        const linkToDeck = selectedOpportunity?.custom_fields.find(cf => cf.custom_field_definition_id===534710)?.value
        const oppRoundSize = selectedOpportunity?.custom_fields.find(cf => cf.custom_field_definition_id===666330)?.value;
        const valuation = selectedOpportunity?.custom_fields.find(cf => cf.custom_field_definition_id===666331)?.value;
        const preOrPostMoneyId = selectedOpportunity?.custom_fields.find(cf => cf.custom_field_definition_id===666332)?.value;
        const roundStructure = roundStructureDisplay(selectedOpportunity?.custom_fields ?? [], roundStructureOptions);
        ...
        return (
            <>
                ...
                {
                    linkToDeck
                    ? <>There is more info about {selectedOpportunity?.name} in the <a href={`${linkToDeck}`} target="_blank">deck</a></>
                    : <>There is more info about {selectedOpportunity?.name} in the attached deck</>
                }
                {
